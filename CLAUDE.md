...
3. Read-time **bridge pooling** (`MuscleStrengthProjector.project`) computes a muscle **level** as the n_eff-weighted average of seed-relative opinions against a seed anchor (levelPrior = 0.5). n_eff = max(0, (1/evidenceVar − 1/σ_seed²) · poolObsVar) with poolObsVar = 2.0e-3 — reading `evidenceVar`, not `sigma2`, so an exercise whose σ was re-opened by adaptive attention keeps its pooling weight instead of being read as uninformed. Each exercise's belief mean is then bridge-shrunk toward the level's prediction by kappa = min(poolObsVar/tauBridge², siblingExcess) with tauBridge = 0.25 — a well-evidenced own measurement (large n_eff) is barely moved; a cold one adopts the prediction fully. The pooled mean and own σ feed `PrescriptionPolicy`. Phase 3 replaces tauBridge with per-equipment-class τ.
...
The tuning constants live in `EstimatorConfig` and are pinned by `BeliefSimulationTest` (re-pinned 2026-07-09 for adaptive attention: obsModelSd 0.02, adaptRunThreshold 2.5, adaptInflationPerExcess 2.0; z/δ/poolObsVar unchanged). The real-history backtest (`ProdBssPrescriptionTest` + the full backtest JVM test) gates each phase's re-baseline (`ProdBssPrescriptionTest` pins the prod BSS at the user-demonstrated 20 lb; the full backtest was re-baselined to the adaptive-attention output 2026-07-09 after attributing the systemic reprice, BAND 0.05). Manual weight overrides write per-exercise `ExerciseStrengthOverride` rows folded in during replay as belief resets at sigmaOverride = 0.10.
