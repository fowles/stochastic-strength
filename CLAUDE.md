...
The tuning constants live in `EstimatorConfig` and are pinned by `BeliefSimulationTest` (re-pinned 2026-07-10 for phase-3 precision-weighted pooling: tauBarbell 0.08, tauMachineCable 0.20, tauOtherLoaded 0.25, levelAnchorPrecision 1.0; obsModelSd 0.02, adaptRunThreshold 2.5, adaptInflationPerExcess 2.0, z/δ unchanged). The real-history backtest (`ProdBssPrescriptionTest` + the full backtest JVM test) gates each phase's re-baseline (`ProdBssPrescriptionTest` pins the prod BSS at the user-demonstrated 20 lb; the full backtest was re-baselined to the phase-3 reliability-weighted-pooling output 2026-07-10 after attributing the systemic upward reprice — real precision-weighted borrowing replacing the old near-zero kappa cap — BAND 0.05). Manual weight overrides write per-exercise `ExerciseStrengthOverride` rows folded in during replay as belief resets at sigmaOverride = 0.10.
