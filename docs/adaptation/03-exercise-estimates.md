...
The **only durable progression state** is one `Belief` per loaded exercise — a running
estimate, a best guess plus how sure we are of it:

```
Belief(mu, sigma2, updatedAt)
```

`mu` is the natural log of the exercise's best-guess fresh 1RM (kg); `sigma2` is how
unsure we are of that guess, in ln-units². Everything lives in log space, so a fixed
percentage move is a fixed additive step. There is no separate confidence score — `sigma2`
*is* the uncertainty: tight (small `sigma2`) means well-evidenced, wide means we don't know
much yet. There is no stored baseline and no stored coefficient.

A brand-new exercise has no estimate at all until it is either seeded (an initial
`ExerciseStrengthOverride` row) or folded for the first time, in which case it borrows its
sibling-pooled prediction as a cold starting guess (see [muscle pooling](04-muscle-pooling.md)).

## Aging — uncertainty grows with idle time

Before any fold, and at read time, an estimate is aged to "now":

```
sigma2 ← (sigma2 + confidenceDecayEstimate · idleDays), clamped to [sigma2Floor, sigma2Cap]
```

`mu` does not drift — an exercise you haven't trained in months keeps its last known
capacity as its best guess, but the growing uncertainty means [pooling](04-muscle-pooling.md)
increasingly defers to its siblings' opinion instead. `confidenceDecayEstimate` is `fitted`
on real history (`BeliefConfig.confidenceDecayEstimate`; curve in the phase-2 plan appendix —
main's old equivalent decay rate was found to be roughly 16× too slow). `sigma2Floor` (±2%)
and `sigma2Cap` (±50%) are `flat` guards: `sigma2` never fully collapses to zero uncertainty,
and never inflates past a ceiling once fully stale.

## Folding a set — a censored (boundary-pull) update

Each set gives one exercise a (fatigue-shifted) implied ln(1RM) interval from
[the signal layer](02-strength-signal.md). `BeliefFold.fold` applies one correction step per
set, in set-id order, aging the estimate to "now" first:

- **Inside the interval** (`mu` already lies within the shifted bounds): the set
  *confirms* what we already think. `mu` is unchanged; `sigma2` shrinks exactly as a
  correction at the nearer boundary would shrink it — confirmation still sharpens our
  confidence even though it doesn't move the best guess.
- **Outside the interval**: `mu` is pulled toward the violated boundary by the correction
  gain `sigma2 / (sigma2 + obsSigma2)`, and `sigma2` shrinks by the same
  confidence-weighted blend.

This is deliberately **symmetric up and down** — a strong RIR-5+ set pulls the estimate up
...
[muscle pooling](04-muscle-pooling.md)). The one per-set doubt estimate, `perSetDoubtEstimate`,
is shared across all feedback buckets — re-confirmed `edge-pinned`/`saturated` at 0.005 on the
...
extreme point on that saturated edge rather than pushed further; see `BeliefConfig.perSetDoubtEstimate`
...
(`PrescriptionPolicy.kt`) — semantic, prescription-time, and separate from the estimate
itself (see [muscle pooling](04-muscle-pooling.md)).

## Folding is local

`BeliefSessionStep.step` folds exercise *i*'s sets into exercise *i*'s estimate only. A
failure on the barbell bench never reaches into the dumbbell fly's stored estimate. That is
...
those seed or reset the estimate directly rather than folding through the set-log path:
...
Both constants are just starting guesses: as soon as real sets are folded against them, the
usual boundary-pull update takes over.
