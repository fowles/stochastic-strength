...
class ExerciseChartGroupingTest {
    ...
    @Test fun setsOfSameSessionCrossingMidnightShareDayKey() {
        ...
        val sessionTimeById = mapOf(1L to sessionStart)
        ...
        val keyBefore = ExerciseChartGrouping.sessionDayKey(beforeMidnight, sessionTimeById, utc)
        val keyAfter = ExerciseChartGrouping.sessionDayKey(afterMidnight, sessionTimeById, utc)
    ...
    @Test fun differentSessionsGetDistinctKeys() {
        val sessionTimeById = mapOf(1L to 10 * msPerDay, 2L to 12 * msPerDay)
        val a = set(sessionId = 1L, completedAt = 10 * msPerDay)
        val b = set(sessionId = 2L, completedAt = 12 * msPerDay)

        assertEquals(10L, ExerciseChartGrouping.sessionDayKey(a, sessionTimeById, utc))
        assertEquals(12L, ExerciseChartGrouping.sessionDayKey(b, sessionTimeById, utc))
    }
    ...
    @Test fun bucketsByLocalCalendarDayNotUtc() {
        ...
        val sessionTimeById = mapOf(1L to startInstant.toEpochMilli())
        ...
        assertEquals(
            ...
            ExerciseChartGrouping.sessionDayKey(s, sessionTimeById, ny),
        )
        // ...whereas UTC would have wrongly put it on Jan 16.
        assertEquals(
            LocalDate.of(2026, 1, 16).toEpochDay(),
            ExerciseChartGrouping.sessionDayKey(s, sessionTimeById, utc),
        )
    }
}
