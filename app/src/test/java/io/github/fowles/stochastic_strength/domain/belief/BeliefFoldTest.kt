...
class BeliefFoldTest {
    ...
    fun agingGrowsVarianceLinearlyPerIdleDay() {
        val b = Belief(bestGuessLn = 4.6f, uncertainty = 0.01f, updatedAt = 0L)
        val aged = fold.aged(b, now = 10 * DAY)
        assertEquals(4.6f, aged.bestGuessLn, 1e-6f)                     // bestGuessLn never drifts
        assertEquals(0.01f + 10 * 1e-3f, aged.uncertainty, 1e-6f)   // q per idle day
        assertEquals(10 * DAY, aged.updatedAt)
    }

    @Test
    fun agingIsClampedToTheVarianceCapAndNeverNegativeTime() {
        val b = Belief(bestGuessLn = 4.6f, uncertainty = 0.24f, updatedAt = 10 * DAY)
        assertEquals(0.25f, fold.aged(b, now = 100 * DAY).uncertainty, 1e-6f)  // cap (flat guard)
        assertEquals(0.24f, fold.aged(b, now = 0L).uncertainty, 1e-6f)         // clock skew: age >= 0
    }
    ...
    fun foldBelowTheIntervalPullsMuUpByKalmanGain() {
        // Prior uncertainty=0.04, obs s2=0.01 → gain = 0.04/0.05 = 0.8; posterior uncertainty = 0.04·0.01/0.05 = 0.008.
        val b = Belief(bestGuessLn = ln(100f), uncertainty = 0.04f, updatedAt = 0L)
        val lower = ln(110f)
        val out = fold.fold(b, LnInterval(lower, null), shift = 0f, obsSigma = 0.1f, at = 5L)
        assertEquals(b.bestGuessLn + 0.8f * (lower - b.bestGuessLn), out.bestGuessLn, 1e-6f)
        assertEquals(0.008f, out.uncertainty, 1e-6f)
    ...
    fun foldAboveTheIntervalPullsMuDownSymmetrically() {
        ...
        val b = Belief(bestGuessLn = ln(120f), uncertainty = 0.04f, updatedAt = 0L)
        val upper = ln(110f)
        val out = fold.fold(b, LnInterval(null, upper), shift = 0f, obsSigma = 0.1f, at = 5L)
        assertEquals(b.bestGuessLn + 0.8f * (upper - b.bestGuessLn), out.bestGuessLn, 1e-6f)
        assertEquals(0.008f, out.uncertainty, 1e-6f)
    }

    @Test
    fun foldInsideTheIntervalConfirmsMuAndOnlyShrinksSigma() {
        val b = Belief(bestGuessLn = ln(105f), uncertainty = 0.04f, updatedAt = 0L)
        val out = fold.fold(b, LnInterval(ln(100f), ln(110f)), shift = 0f, obsSigma = 0.1f, at = 5L)
        assertEquals(b.bestGuessLn, out.bestGuessLn, 1e-7f)
        assertEquals(0.008f, out.uncertainty, 1e-6f)   // same shrink a boundary fold would give
    }

    @Test
    fun shiftMovesTheIntervalUpBeforeComparing() {
        // bestGuessLn inside the raw interval but below it once shifted: a fatigued success implies MORE fresh capacity.
        val b = Belief(bestGuessLn = ln(100f), uncertainty = 0.04f, updatedAt = 0L)
        val raw = LnInterval(ln(99f), ln(101f))
        val shift = 0.10f
        val out = fold.fold(b, raw, shift = shift, obsSigma = 0.1f, at = 5L)
        assertEquals(b.bestGuessLn + 0.8f * (ln(99f) + shift - b.bestGuessLn), out.bestGuessLn, 1e-6f)
    }
    ...
    fun foldSessionRanksAllRowsButFoldsOnlyScoreableOnes() {
        ...
        val prior = Belief(bestGuessLn = ln(100f), uncertainty = 0.01f, updatedAt = 0L)
