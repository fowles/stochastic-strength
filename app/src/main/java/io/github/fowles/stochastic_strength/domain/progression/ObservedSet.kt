 ...
 *
 * RIR feedbacks add the same reserve offsets used for the implied 1RM and
...
fun impliedObservedSet(set: WorkoutSet): ObservedSet? {
    ...
    val isEstimate: Boolean
    when (feedback) {
        SetFeedback.RIR_0_1 -> { reps = (set.targetReps + DISPLAY_RESERVE_RIR_0_1).roundToInt(); isEstimate = true }
        SetFeedback.RIR_2_4 -> { reps = (set.targetReps + DISPLAY_RESERVE_RIR_2_4).roundToInt(); isEstimate = true }
        SetFeedback.RIR_5_PLUS -> { reps = (set.targetReps + DISPLAY_RESERVE_RIR_5_PLUS).roundToInt(); isEstimate = true }
