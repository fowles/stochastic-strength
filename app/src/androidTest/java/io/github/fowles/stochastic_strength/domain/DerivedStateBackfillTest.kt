...
class DerivedStateBackfillTest {
    ...
    private suspend fun seedFullReplayData() {
        // exercise + per-muscle baseline_override initial + completed session with sets. The
        // per-muscle override is expanded live (ExerciseSeedExpansion) into a per-exercise seed
        // during replay, producing baseline_history rows. Name must resolve to a seed coefficient
